 ...
 */
export const updatePropertyGlobal = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (filteredUpdateData.owner !== undefined) {
      if (filteredUpdateData.owner === 'admin') {
        ...
        filteredUpdateData.owner = new ObjectId(filteredUpdateData.owner);
        filteredUpdateData.isAdminOwned = false;
      }
    }
    ...
    if (!property) {
      ...
      property = await db.collection('properties').findOne({ 
        _id: new ObjectId(actualPropertyId)
      });
    }
 ...
 */
export const updatePropertyOriginal = async (req: Request, res: Response) => {
  const startTime = Date.now();
  ...
  try {
    ...
    const updatedProperty = await Property.default.findOneAndUpdate(
      { _id: property._id },
      { 
        ...filteredUpdateData,
        updatedAt: new Date()
