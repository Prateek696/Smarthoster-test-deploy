...
import { 
  getHostkitReservations,
  updateHostkitCalendar 
} from "../integrations/hostkit.api";

// Get advanced calendar events (bookings, blocks, pricing)
export const getAdvancedCalendarEvents = async (req: Request, res: Response) => {
  try {
    const { propertyId, startDate, endDate } = req.query as { 
      propertyId: string; 
      startDate: string; 
      endDate: string; 
    };
    ...
    try {
      hostawayCalendar = await getHostawayCalendar(listingId, startDate, endDate);
    ...
    try {
      hostawayReservations = await getHostawayReservations(listingId, startDate, endDate);
    ...
    try {
      hostkitReservations = await getHostkitReservations(listingId, startDate, endDate);
    ...
    if (hostawayCalendar.result && Array.isArray(hostawayCalendar.result)) {
      hostawayCalendar.result.forEach((item: any) => {
        try {
    ...
    if (hostawayReservations.data && Array.isArray(hostawayReservations.data)) {
      hostawayReservations.data.forEach((reservation: any) => {
        events.push({
    ...
    if (hostkitReservations && Array.isArray(hostkitReservations)) {
      hostkitReservations.forEach((reservation: any) => {
        events.push({
...
export const createAdvancedCalendarEvent = async (req: Request, res: Response) => {
  try {
    ...
    if (type === 'block') {
      ...
      await updateHostawayCalendar(listingId, startDate, endDate, 'blocked');
      
      // Mirror to Hostkit if available
      try {
        await updateHostkitCalendar(listingId, startDate, endDate, 'blocked');
      } catch (error) {
...
export const updateAdvancedCalendarEvent = async (req: Request, res: Response) => {
  try {
...
export const deleteAdvancedCalendarEvent = async (req: Request, res: Response) => {
  try {
    const { eventId } = req.params;
...
// Get bulk operations
export const getBulkOperations = async (req: Request, res: Response) => {
  try {
...
export const createBulkOperation = async (req: Request, res: Response) => {
  try {
