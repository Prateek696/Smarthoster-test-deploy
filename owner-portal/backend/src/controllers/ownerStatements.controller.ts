...
import { calculateOwnerStatements } from '../services/ownerStatements.service';
import Property from '../models/property.model';

export const getOwnerStatements = async (req: Request, res: Response) => {
  try {
    ...
    const commission = property.isAdminOwned ? 0 : (commissionPercentage ? parseInt(commissionPercentage as string) : 25);
    const result = await calculateOwnerStatements(
      parseInt(propertyId),
      startDateStr,
      endDateStr,
      commission
