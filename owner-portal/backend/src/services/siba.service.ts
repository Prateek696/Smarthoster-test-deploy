...
import { getHostkitReservations, getLastHostkitSibaDate } from "../integrations/hostkit.api";
...
export const getSibaStatusService = async (propertyId: number) => {
  try {
    ...
    let lastSibaSendDate: string | null = null;
    
    try {
      const hostkitSibaData = await getLastHostkitSibaDate(propertyId);
    ...
    if (!lastSibaSendDate) {
      try {
        ...
        const [hostkitReservations, hostawayReservations] = await Promise.all([
          getHostkitReservations(propertyId, startDate, endDate).catch((error) => {
            console.log(`Hostkit reservations error for property ${propertyId}:`, error.message);
            return [];
          }),
          getHostawayReservations(propertyId, startDate, endDate).catch((error) => {
        ...
        const reservationsWithSiba = allReservations.filter((reservation) => {
          const foundFields = sibaFieldVariations.filter(field => 
            reservation[field] && reservation[field] !== null && reservation[field] !== ''
          );
          ...
          const sortedBySibaDate = reservationsWithSiba.sort((a, b) => {
            const getDate = (res: any) => {
              for (const field of sibaFieldVariations) {
                if (res[field]) return new Date(res[field]);
              }
              return new Date(0);
            };
...
const getHostkitSibaStatus = async (propertyId: number, startDate: string, endDate: string) => {
  try {
    const dateStart = Math.floor(new Date(startDate).getTime() / 1000);
    const dateEnd = Math.floor(new Date(endDate).getTime() / 1000);

    const { data } = await axios.get(`${HOSTKIT_API_URL}/getSibaStatus`, {
      params: {
...
export const validateSibaService = async (reservationId: number) => {
  const { data } = await axios.get(`${MUNICIPAL_API_URL}/validate/${reservationId}`);
...
export const sendSibaService = async (reservationId: number, propertyId: number) => {
  try {
    ...
    try {
      municipalResponse = await axios.post(`${MUNICIPAL_API_URL}/submit`, {
      reservationId,
      propertyId,
        guestData: {
...
const getReservationDetails = async (reservationId: number, propertyId: number) => {
  try {
    // Try Hostkit first
    const hostkitReservations = await getHostkitReservations(propertyId, 
      new Date(Date.now() - 365 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 1 year ago
      new Date().toISOString().split('T')[0]
    );
    
    let reservation = hostkitReservations.find((r: any) => r.id === reservationId || r.reservationId === reservationId);
    
    if (!reservation) {
      // Try Hostaway
      const hostawayReservations = await getHostawayReservations(propertyId,
        new Date(Date.now() - 365 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
        new Date().toISOString().split('T')[0]
      );
      
      reservation = hostawayReservations.result?.find((r: any) => r.id === reservationId);
    }
...
const validateReservationForSiba = async (reservation: any) => {
  const errors: string[] = [];
  
  if (!reservation.guestName) errors.push("Guest name is required");
  if (!reservation.guestDocument && !reservation.guestDocumentNumber) errors.push("Guest document number is required");
  if (!reservation.checkIn || !reservation.arrivalDate) errors.push("Check-in date is required");
  if (!reservation.checkOut || !reservation.departureDate) errors.push("Check-out date is required");
  if (!reservation.adults || reservation.adults < 1) errors.push("Number of adults is required");
...
const updateHostkitSibaStatus = async (reservationId: number, propertyId: number, status: string, message: string) => {
  try {
    await axios.post(`${HOSTKIT_API_URL}/updateSibaStatus`, {
      APIKEY: HOSTKIT_API_KEY,
...
export const bulkSibaStatusService = async (propertyIds: number[]) => {
  const results = await Promise.all(propertyIds.map(pid => getSibaStatusService(pid)));
  return results;
};
