...
import { getHostkitInvoices, getHostkitReservations } from "../integrations/hostkit.api";

export const getMonthlyPerformanceService = async (
  listingId: number,
  month: string
) => {
  try {
    ...
    const [hostawayReservations, hostkitReservations, invoicesResult] = await Promise.all([
      getHostawayReservations(listingId, dateStart, dateEnd).catch(() => ({ result: [] })),
      getHostkitReservations(listingId, dateStart, dateEnd).catch(() => []),
      getHostkitInvoices(listingId, dateStart, dateEnd).catch(() => [])
    ]);

    // Extract reservations arrays safely
    const hostawayRes: any[] = Array.isArray(hostawayReservations.result)
      ? hostawayReservations.result
      : [];
      
    const hostkitRes: any[] = Array.isArray(hostkitReservations) 
      ? hostkitReservations 
      : [];
    ...
    const hostawayFiltered = hostawayRes.filter(r => {
      ...
      const overlapsMonth =
        (arrival >= fromDate && arrival <= toDate) ||
    ...
    const hostkitFiltered = hostkitRes.filter((r: any) => {
    ...
    const invoiceTotal = invoices.reduce((sum: number, inv: any) => 
      sum + (parseFloat(inv.value) || 0), 0
    );
