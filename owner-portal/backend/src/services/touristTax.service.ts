import { getChannelBookings, getHostkitTouristTax, getHostkitReservations } from "../integrations/hostkit.api";
import { filterBookings } from "../utils/dataFilter";
...
export const getCityTaxService = async (
  propertyId: number,
  startDate: string,
  endDate: string,
  filterType: 'checkin' | 'checkout' = 'checkin'
) => {
  try {
    ...
    try {
      ...
      const reservationsData = await getHostkitReservations(propertyId, startDate, endDate);
      ...
      console.log('Reservations API failed, trying bookings API...', (reservationError as Error).message);
      // Fallback to bookings API
      bookings = await getChannelBookings(propertyId, startDate, endDate);
    }
    ...
    const excludedBookings = bookings.filter((booking: any) => {
      let checkInDate: Date | null = null;
      let checkOutDate: Date | null = null;
    ...
    excludedBookings.slice(0, 5).forEach((booking: any) => {
    ...
    const stats = {
      ...
      bookingsPlatforms: {} as { [key: string]: number },
    ...
    filteredBookings.forEach((booking: any) => {
      ...
      if (booking.guest_data && Array.isArray(booking.guest_data)) {
        ...
        booking.guest_data.forEach((guest: any) => {
          if (guest.birthday) {
    ...
    try {
      const apiTax = await getHostkitTouristTax(propertyId, startDate, endDate);
      if (apiTax > 0) {
...
export const getTouristTaxDetailedService = async (
  propertyId: number,
  startDate: string,
  endDate: string,
  filterType: 'checkin' | 'checkout' = 'checkout'
) => {
  try {
    // Use the working reservations API instead of problematic bookings API
    let bookings: any[] = [];
    ...
    const taxDetails = filteredBookings.map((booking: any) => {
      // Guest calculations - handle different API field names from Hostkit
      let adults = 0;
      let children = 0;
    ...
    // Platform breakdown
    const platformBreakdown = taxDetails.reduce((acc: any, detail) => {
