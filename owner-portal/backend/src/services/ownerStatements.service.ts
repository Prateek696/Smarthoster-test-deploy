...
export interface OwnerStatementSummary {
  ...
  management_commission_base: number;
...
export const calculateOwnerStatements = async (
  ...
  try {
    ...
    if (filteredInvoices.length > 0) {
      ...
      const total_invoice_vat = filteredInvoices.reduce((sum, invoice: any) => {
        let invoiceVat = 0;
      ...
      const total_invoice_gross = filteredInvoices.reduce((sum, invoice: any) => {
