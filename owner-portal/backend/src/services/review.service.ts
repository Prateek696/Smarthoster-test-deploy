...
import { getHostawayReviews, respondToHostawayReview } from '../integrations/hostaway.api';
import { getPropertyMapping, PropertyMapping } from './propertyMapping.service';

// Fetch reviews from Hostaway API
const fetchReviewsFromAllPlatforms = async (propertyMapping: PropertyMapping, limit: number) => {
  ...
  try {
    ...
    try {
      ...
      const hostawayReviewsResponse = await getHostawayReviews(propertyMapping.internalId, limit, 0);
      ...
      if (hostawayReviewsResponse && hostawayReviewsResponse.result && Array.isArray(hostawayReviewsResponse.result)) {
        // First filter by property (listingMapId)
                const propertyReviews = hostawayReviewsResponse.result.filter((review: any) =>
          review.listingMapId === propertyMapping.internalId
        );
        ...
        // Then filter only guest-to-host reviews with ratings
                const guestReviews = propertyReviews.filter((review: any) =>
        ...
        const hostawayReviews = guestReviews.map((review: any) => ({
...
export const getReviews = async (propertyId: number, limit: number = 10) => {
  ...
  try {
    ...
    if (!propertyMapping) {
      ...
      const dbReviews = await ReviewModel.find({ propertyId })
        .sort({ reviewDate: -1 })
        .limit(limit);
    ...
    for (const review of reviews) {
      const existingReview = await ReviewModel.findOne({ reviewId: review.reviewId });
      if (!existingReview) {
...
export const getReviewStats = async (propertyId: number) => {
  console.log(`[REVIEW STATS] Fetching stats for property ${propertyId}`);
  
  // Get reviews from the same source as getReviews
  const reviews = await getReviews(propertyId, 100); // Get more reviews for stats
...
export const addReviewResponse = async (
  ...
  try {
    ...
    if (reviewId.startsWith('hostaway-')) {
      ...
      platformResponse = await respondToHostawayReview(hostawayReviewId, responseText);
...
export const syncReviewsFromPlatforms = async (propertyId: number) => {
  try {
    const propertyMapping = getPropertyMapping(propertyId);
    if (!propertyMapping) {
    ...
    for (const reviewData of reviews) {
      const existingReview = await ReviewModel.findOne({ reviewId: reviewData.reviewId });
      if (!existingReview) {
