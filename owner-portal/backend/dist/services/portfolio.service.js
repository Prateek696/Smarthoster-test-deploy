...
exports.getPortfolioTrends = exports.getPortfolioOverview = void 0;
const performance_service_1 = require("./performance.service");
const booking_service_1 = require("./booking.service");
const touristTax_service_1 = require("./touristTax.service");
const getPortfolioOverview = async (propertyIds, month) => {
    try {
        const portfolioData = await Promise.all(propertyIds.map(async (propertyId) => {
            try {
                // Get performance data for each property
                const performance = await (0, performance_service_1.getMonthlyPerformanceService)(propertyId, month);
                // Get booking data for occupancy calculation
                const startDate = `${month}-01`;
                const endDate = `${month}-31`;
                const bookings = await (0, booking_service_1.getBookingsService)(propertyId, startDate, endDate);
                // Get tourist tax data
                const touristTax = await (0, touristTax_service_1.getCityTaxService)(propertyId, startDate, endDate);
...
const getPortfolioTrends = async (propertyIds, months) => {
    try {
        const trendsData = await Promise.all(months.map(async (month) => {
            const portfolioData = await (0, exports.getPortfolioOverview)(propertyIds, month);
            return {
                month,
