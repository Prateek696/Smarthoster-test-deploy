...
exports.generateMonthlyPDF = exports.getMonthlyStatementSummary = exports.downloadOwnerStatementCSV = exports.downloadOwnerStatementPDF = exports.getOwnerStatement = exports.getAllOwnerStatements = exports.getOwnerStatements = void 0;
const hostaway_api_1 = require("../integrations/hostaway.api");
const simplePDFGenerator_service_1 = require("./simplePDFGenerator.service");
const invoice_service_1 = require("./invoice.service");
const expense_service_1 = require("./expense.service");
const hostkit_api_1 = require("../integrations/hostkit.api");
// Helper function to get real statement data
async function getRealStatementData(propertyId, year, month) {
    const startDate = `${year}-${month.toString().padStart(2, '0')}-01`;
    const endDate = `${year}-${month.toString().padStart(2, '0')}-${new Date(year, month, 0).getDate()}`;
    try {
        console.log(`[REAL DATA] Fetching invoices for property ${propertyId} from ${startDate} to ${endDate}`);
        // Get invoices
        const invoices = await (0, invoice_service_1.getInvoicesService)(propertyId, startDate, endDate);
        ...
        try {
            const hostawayExpensesResponse = await (0, hostaway_api_1.getHostawayExpenses)(propertyId, startDate, endDate);
            ...
            console.log(`[REAL DATA] Hostaway expenses not available:`, hostawayError.message);
            expensesAvailable = false;
        }
        // Get local expenses (fallback)
        const localExpenses = await (0, expense_service_1.fetchExpenses)(propertyId, year, month);
        ...
        let reservations = [];
        try {
            const reservationsResponse = await (0, hostkit_api_1.getHostkitReservations)(propertyId, startDate, endDate);
            ...
            console.log(`[REAL DATA] Reservations not available:`, reservationError.message);
        }
        ...
        const vat = invoices.reduce((sum, inv) => {
        ...
        const paidInvoices = invoices.filter((inv) => inv.closed === true || inv.closed === 1).length;
        const partialInvoices = invoices.filter((inv) => inv.partial === true || inv.partial === 1).length;
        const pendingInvoices = totalInvoices - paidInvoices - partialInvoices;
        // Calculate average invoice value
        const averageInvoiceValue = totalInvoices > 0 ? totalRevenue / totalInvoices : 0;
        // Calculate Hostaway expenses
        const hostawayExpensesTotal = hostawayExpenses.reduce((sum, exp) => {
            const amount = parseFloat(exp.amount || '0');
            return sum + (isNaN(amount) ? 0 : amount);
        }, 0);
        // Calculate local expenses
        const localExpensesTotal = localExpenses.reduce((sum, exp) => {
            const amount = parseFloat(exp.amount || '0');
            return sum + (isNaN(amount) ? 0 : amount);
        }, 0);
        // Calculate cleaning fees from reservations
        const cleaningFees = reservations.reduce((sum, res) => {
        ...
        const platformFees = reservations.reduce((sum, res) => {
...
const getOwnerStatements = async (propertyId, startDate, endDate, limit = 50, offset = 0, selectedYear, selectedMonth) => {
    try {
        ...
        try {
            ...
            const hostawayResponse = await (0, hostaway_api_1.getHostawayOwnerStatements)(propertyId, startDate, endDate, limit, offset);
            if (hostawayResponse && hostawayResponse.result && Array.isArray(hostawayResponse.result)) {
                const statements = hostawayResponse.result.map((statement) => ({
                    id: statement.id,
...
const getAllOwnerStatements = async (startDate, endDate, limit = 100, offset = 0) => {
    try {
        console.log(`[OWNER STATEMENT SERVICE] Fetching all statements`);
        const hostawayResponse = await (0, hostaway_api_1.getHostawayOwnerStatements)(undefined, // No specific property
...
exports.getAllOwnerStatements = getAllOwnerStatements;
// Get specific owner statement by ID
const getOwnerStatement = async (statementId) => {
    try {
        console.log(`[OWNER STATEMENT SERVICE] Fetching statement ${statementId}`);
        const hostawayResponse = await (0, hostaway_api_1.getHostawayOwnerStatement)(statementId);
...
exports.getOwnerStatement = getOwnerStatement;
// Download owner statement as PDF
const downloadOwnerStatementPDF = async (statementId) => {
    try {
        console.log(`[OWNER STATEMENT SERVICE] Downloading PDF for statement ${statementId}`);
        // First try to get from Hostaway API
        try {
            const pdfBuffer = await (0, hostaway_api_1.downloadHostawayOwnerStatementPDF)(statementId);
            return pdfBuffer;
        }
        catch (apiError) {
            ...
            const statement = await (0, exports.getOwnerStatement)(statementId);
            if (!statement) {
            ...
            return simplePDFGenerator_service_1.SimplePDFGenerator.generatePDF(statementData);
        }
    }
    catch (error) {
...
exports.downloadOwnerStatementPDF = downloadOwnerStatementPDF;
// Download owner statement as CSV
const downloadOwnerStatementCSV = async (statementId) => {
    try {
        console.log(`[OWNER STATEMENT SERVICE] Downloading CSV for statement ${statementId}`);
        // First try to get from Hostaway API
        try {
            const csvBuffer = await (0, hostaway_api_1.downloadHostawayOwnerStatementCSV)(statementId);
            return csvBuffer;
        }
        catch (apiError) {
            ...
            return simplePDFGenerator_service_1.SimplePDFGenerator.generateCSV(statementData);
        }
    }
    catch (error) {
...
exports.downloadOwnerStatementCSV = downloadOwnerStatementCSV;
// Generate monthly statement summary using real invoice data
const getMonthlyStatementSummary = async (propertyId, year, month) => {
    try {
        ...
        try {
            ...
            const statements = await (0, exports.getOwnerStatements)(propertyId, startDate, endDate, 100, 0, year, month);
...
exports.getMonthlyStatementSummary = getMonthlyStatementSummary;
// Generate monthly PDF statement
const generateMonthlyPDF = async (propertyId, propertyName, year, month) => {
    try {
        console.log(`[OWNER STATEMENT SERVICE] Generating monthly PDF for property ${propertyId}, ${year}-${month}`);
        const statements = await (0, exports.getOwnerStatements)(propertyId, undefined, undefined, 100, 0, year, month);
        ...
        return simplePDFGenerator_service_1.SimplePDFGenerator.generatePDF(monthlyData);
    }
    catch (error) {
